...
import { _decorator, Component, Node,ScrollView,Enum,Size,EventHandler } from 'cc';
...
export default class TableView extends Component {
    ...
    private _updateContentSize(isUseLastOffset: boolean) {
        ...
        if (!isUseLastOffset) {
            ...
            //TODO 左右滑动时不知为何scrollOffset获取到是负的 这里修改成正的
            if(this.scrollView.horizontal){
                scrollOffset = new Vec2(-scrollOffset.x,scrollOffset.y);
            }
            this.scrollView.scrollToOffset(scrollOffset, 0.01);
        }
    }

    //索引转坐标
    private _positionFromIndex(index: any): Vec2 {
    ...
    private _setIndexForCell(index: number, cell: TableViewCellNode) {
        ...
        let v2 = this._positionFromIndex(index);
        cell.setPosition(v2.x,v2.y);
        cell.setIdx(index);
    }
    ...
    //创建索引处的单元格
    private _createCellAtIndex(idx:any): TableViewCellNode {
